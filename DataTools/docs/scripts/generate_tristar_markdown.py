...
def _build_markdown(title: str, image_paths: list[Path]) -> str:
    ...
    lines.append("   Serial: CI000042  MAC: DE:AD:BE:EF:00:00")
    ...
    lines.append("   Serial: CI000042  MAC: F0:D7:AF:49:23:DA")
    ...
    lines.append("```")
    lines.append("1. [Searching...]  → No device connected yet")
    lines.append("2. [Connected] SubPro-XYZ789  → Device found")
    ...
    lines.append("```")
    lines.append("")
    lines.append("**Device already provisioned (no re-provisioning):**")
    lines.append("")
    lines.append("```")
    lines.append("1. [Searching...]  → No device connected yet")
    lines.append("2. [Connected] SubPro-DEFGH  → Device found")
    lines.append("   Serial: CJ000055  MAC: F0:D7:AF:49:23:DB")
    lines.append("3. [OK] Already provisioned. MAC: F0:D7:AF:49:23:DB. Disconnect to provision next.")
    lines.append("   (Device already in database with matching MAC - no action needed)")
    lines.append("4. (User disconnects device)")
    lines.append("5. [Searching...]  → Ready for next device")
    lines.append("```")
    lines.append("")
